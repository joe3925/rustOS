...
impl PELoader {
    ...
    pub fn is_pic(&self) -> bool {
        let dynbase = {
            let dll_chars = self.pe.header.optional_header
                            .as_ref()
    ...
    pub fn reloc_table(&self) -> Option<impl Iterator<Item = RelocationEntry> + '_> {
       let section = self.pe.sections.iter()
    ...
    pub fn dll_load(&mut self, pid: u64) -> Result<Module, LoadError>{
        ...
        let opt_hdr = self.pe.header.optional_header.as_ref().ok_or(LoadError::MissingSections)?;
        let image_size = opt_hdr.windows_fields.size_of_image as u64;


        unsafe { Cr3::write(cr3, Cr3::read().1) };
        if(program.tracker.alloc(opt_hdr.windows_fields.image_base, image_size).is_err() || self.needs_relocation()){
        ...
        unsafe { program.virtual_map(VirtAddr::new(opt_hdr.windows_fields.image_base), image_size as usize) };
        self.load_sections();
        let module = Module{
            title: File::remove_file_from_path(self.path.as_str()).to_string(),
            image_path: self.path.clone(),
    ...
    pub fn load(&mut self) -> Result<u64, LoadError> {
        ...
        match program.virtual_map_alloc(program.image_base, (image_size + 0x1000 + stack_size + heap_size ) as usize){
            Err(MapToError::FrameAllocationFailed) => return Err(LoadError::NoMemory),
            Err(_) => (),
            Ok(_) => ()
        }

        let thread = Task::new_usermode((self.pe.entry as i64 + self.current_base.as_u64() as i64) as usize, stack_size, File::remove_file_from_path(self.path.as_str()).to_string(), stack_addr, 0);
    ...
    pub fn relocate(&mut self) -> Result<(), LoadError> {
        ...
        let relocation_table = self.reloc_table().ok_or(LoadError::UnsupportedRelocationFormat)?;
    ...
    pub fn calculate_relocation_base(&mut self, range_tracker: &RangeTracker) -> Result<VirtAddr, LoadError>{
        let opt_hdr = self.pe.header.optional_header.ok_or(LoadError::MissingSections)?;
        ...
        let new_base = range_tracker.alloc_auto(alloc_size as u64).ok_or(LoadError::NoMemory)?;
