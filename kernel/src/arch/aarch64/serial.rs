...
#[inline]
fn try_write_byte_unlocked(byte: u8) -> bool {
    if unsafe { register(UART_FLAGS).read_volatile() } & UART_TX_FULL != 0 {
        false
    } else {
        unsafe {
            register(UART_DATA).write_volatile(byte as u32);
        }
...
fn write_byte_unlocked(byte: u8) {
    while !try_write_byte_unlocked(byte) {
        core::hint::spin_loop();
    }
}
...
pub(crate) fn try_write_byte(byte: u8) -> bool {
    let Some(_guard) = WRITE_LOCK.try_lock() else {
        return false;
    };

    try_write_byte_unlocked(byte)
}
...
pub(crate) fn write_metadata_bytes(bytes: &[u8]) {
    ...
    for (index, &byte) in bytes.iter().enumerate() {
        ...
        write_bytes_unlocked(META_PREFIX);
        write_bytes_unlocked(&bytes[start..=index]);
    ...
    if start < bytes.len() {
        write_bytes_unlocked(META_PREFIX);
        write_bytes_unlocked(&bytes[start..]);
    }
}
