...
fn execute_codes(
    ...
    for _ in 0..256 {
        ...
        match opcode {
            0x00..=0x1f => {
                context.sp = add_sp(context.sp, (opcode & 31) as u64 * 16)?;
            }
            ...
            0x40..=0x7f => {
                let address = context
                    .sp
                    .checked_add((opcode & 63) as u64 * 8)
                    .ok_or(STATUS_BAD_STACK_READ)?;

                restore_fplr(context, bounds, address)?;
            }
            ...
            0xc0..=0xc7 => {
                let amount = ((((opcode & 7) as u64) << 8) | bytes[1] as u64) * 16;

                context.sp = add_sp(context.sp, amount)?;
            }
            ...
            0xe0 => {
                let amount = u32::from_be_bytes([0, bytes[1], bytes[2], bytes[3]]) as u64 * 16;
            ...
            0xe6 => {
                let pair = save_next_target(context, codes, index)?;

                if matches!(pair.kind, SavedPairKind::Integer) {
                    restore_pair(context, bounds, pair.reg, pair.address)?;
                }
            }

            0xe7 => {
                execute_save_any(context, bounds, bytes)?;
            }
...
fn save_next_target(
    ...
    codes: &[u8],
    mut index: usize,
) -> Result<SavedPair, u32> {
    let mut distance = 1u8;

    while *codes.get(index).ok_or(STATUS_BAD_UNWIND_INFO)? == 0xe6 {
        distance = distance.checked_add(1).ok_or(STATUS_BAD_UNWIND_INFO)?;

        index = index.checked_add(1).ok_or(STATUS_BAD_UNWIND_INFO)?;
    }

    let base = decode_pair_base(context, codes, index)?;

    let register_offset = distance.checked_mul(2).ok_or(STATUS_BAD_UNWIND_INFO)?;
    ...
    let address_offset = base
        .stride
        .checked_mul(distance as u64)
        .ok_or(STATUS_BAD_STACK_READ)?;
...
fn decode_pair_base(context: &UnwindContext, codes: &[u8], index: usize) -> Result<SavedPair, u32> {
    let opcode = *codes.get(index).ok_or(STATUS_BAD_UNWIND_INFO)?;
    ...
    match opcode {
        0x20..=0x3f => Ok(SavedPair {
            kind: SavedPairKind::Integer,
            reg: 19,
            address: context.sp,
            stride: 16,
        }),
        ...
        0xe7 => {
            if bytes.len() != 3 || bytes[1] & 0x80 != 0 || bytes[1] & 0x40 == 0 {
                return Err(STATUS_BAD_UNWIND_INFO);
            }
            ...
            let offset = (bytes[2] & 0x3f) as u64 * 16;
            ...
            let (kind, stride) = match kind {
                0 => (SavedPairKind::Integer, 16),
                1 => (SavedPairKind::Float64, 16),
                2 => (SavedPairKind::Float128, 32),
                3 => {
                    return Err(STATUS_UNSUPPORTED_OPCODE);
                }
                _ => unreachable!(),
...
fn execute_save_any(
    ...
    if kind == 0 {
        ...
        if pair {
            let next_reg = reg.checked_add(1).ok_or(STATUS_BAD_UNWIND_INFO)?;

            restore_reg(context, bounds, next_reg, address + 8)?;
        }
    }
...
impl UnwindContext {
    ...
    fn set_reg(&mut self, reg: u8, value: u64) {
        match reg {
            19..=28 => {
                self.registers[(reg - 19) as usize] = value;
            }
