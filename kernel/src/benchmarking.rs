...
fn append_to_file(path: &str, data: &[u8]) -> Result<(), ()> {
    let _ = File::make_dir(path.to_string());

    let file_path = alloc::format!("{path}\\benchmark.txt");

    let mut file = match File::open(&file_path, &[OpenFlags::Create]) {
        Ok(f) => f,
        Err(_) => match File::open(&file_path, &[OpenFlags::Open]) {
            Ok(f2) => f2,
            Err(_) => return Err(()),
        },
    };
    ...
    let mut merged = Vec::with_capacity(existing.len() + data.len());
    merged.extend_from_slice(&existing);
    merged.extend_from_slice(data);

    if let Err(_) = file.write(&merged) {
        return Err(());
    }
