...
use kernel_api::{
    ...
    pnp::{
        DeviceRelationType, DriverStep, PnpOp, PnpOps, QueryDeviceRelations, StartDevice,
        driver_set_evt_device_add, get_device_tree_blob, get_rsdp,
...
#[request_handler]
pub async fn root_query_devrels(
...
fn ensure_acpi_node(parent: &Arc<DevNode>) {
    ...
    let _ = pnp_create_child_devnode_and_pdo_with_init(
        ...
        ids,
        None,
        DeviceInit::with_pnp(Some(PnpOps::new())),
    );
}
