...
pub const HEAP_SIZE: usize = 20000 * 1024;
pub(crate) fn init_heap(mapper: &mut impl Mapper<Size4KiB>, frame_allocator: &mut impl FrameAllocator<Size4KiB>){
    ...
    let frame = frame_allocator
        .allocate_frame().expect("failed to alloc heap frame");
    while (current_page <= end_page){
        let flags = PageTableFlags::PRESENT | PageTableFlags::WRITABLE;
        unsafe { mapper.map_to(current_page, frame, flags, frame_allocator).expect("TODO: panic message"); }
        current_page += 1;
    }
